...
/*Index's*/
CREATE INDEX idx_Agil_core ON Productos(id_prod); /*Hemos pensado que era el mas basico, ya que agiliza la primary key del core*/
CREATE INDEX idx_Buscar_stock ON Productos(Stock); /*Cuando el usuario compra, para saber si hay stock de x articulo, pensamos que es una busqueda que se realiza muchas veces*/
